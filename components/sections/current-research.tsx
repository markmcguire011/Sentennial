...
export default function CurrentResearch({ series }: Props) {
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
            whileInView={{ width: `${series.progress}%` }}
        ...
        <div className="flex justify-between items-center pt-2">
          {series.estimatedCompletion && (
            <div className="text-xs opacity-50 pt-2">
              Est. completion: {series.estimatedCompletion}
            </div>
          )}
          {series.link && (
            <div className="flex items-center gap-1 text-xs text-brand-color pt-2">
              <Link
                className="opacity-50 font-underline group-hover:opacity-100"
                href={series.link}
              >
                View series
              </Link>
